...
export default function SteelCutCalculatorPage() {
  ...
  const calculateWasteRects = (basePlate, placedPieces) => {
    let freeRects = [
      { x: 0, y: 0, width: basePlate.width, height: basePlate.height },
    ];
      ...
      freeRects.forEach((freeRect) => {
        ...
        if (piece.y + piece.height < freeRect.y + freeRect.height) {
          nextFreeRects.push({
            ...
            width: freeRect.width,
            height: freeRect.y + freeRect.height - (piece.y + piece.height),
          });
        }
        // Left part
        ...
        if (piece.x + piece.width < freeRect.x + freeRect.width) {
          nextFreeRects.push({
            ...
            y: piece.y,
            width: freeRect.x + freeRect.width - (piece.x + piece.width),
            height: piece.height,
          });
        }
      });
    ...
    setTimeout(() => {
      ...
      while (piecesToPlace.length > 0) {
        ...
        const canPlace = (piece, x, y) => {
          if (
            x + piece.width > basePlate.width ||
            y + piece.height > basePlate.height
          )
            return false;
          for (const rect of placedRects) {
            if (
              x < rect.x + rect.width &&
              x + piece.width > rect.x &&
              y < rect.y + rect.height &&
              y + piece.height > rect.y
            ) {
              return false;
            }
          }
        ...
        if (currentPlateLayout.length > 0) {
          ...
          const plateUsedArea = currentPlateLayout.reduce(
      ...
      const totalBaseArea =
        allLayouts.length * basePlate.width * basePlate.height;
      ...
      >
        ...
        >
          ...
          <rect
            x="0"
            y="0"
            width={rotatedViewBoxWidth}
            height={rotatedViewBoxHeight}
            fill="url(#grid)"
            stroke="#9ca3af"
            strokeWidth="4"
          />
          ...
          <div className="flex justify-between items-center no-print">
            ...
            >
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M5 4v3H4a2 2 0 00-2 2v6a2 2 0 002 2h1v-4a1 1 0 011-1h8a1 1 0 011 1v4h1a2 2 0 002-2v-6a2 2 0 00-2-2h-1V4a2 2 0 00-2-2H7a2 2 0 00-2 2zm8 0H7v3h6V4zm0 8H7v4h6v-4z"
                  clipRule="evenodd"
                />
              </svg>
